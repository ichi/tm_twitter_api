...
var TwitterApi = function(params){
    ...
    if (oAuthAdapter.isAuthorized() == false) 
    {
      var receivePin = function() {
        oAuthAdapter.getAccessToken('https://api.twitter.com/oauth/access_token', function(){
          ...
          if(callback) callback.call(self, oAuthAdapter);
        });
      };
  ...
  this.readAccessTokenAndSecret = function(){
    return self.oAuthAdapter.readAccessTokenAndSecret('twitter');
  };
  ...
  */
  ...
  this.statuses_update = function(params){
    ...
    params.method = 'POST';
    params = update_params(params, ['lat', 'long', 'place_id', 'display_coordinates', 'trim_user', 'include_entities']);
    return self.callApi(params);
  };
  //statuses.destroy
  ...
  var update_params = function(params, keys){
    var queries = [];
    keys.forEach(function(key){
        if(params.parameters[key]){
            queries.push([key, params.parameters[key]].join('='));
            delete params.parameters[key];
        }
    });
    params.url = [params.url, queries.join('&')].join('?');
    return params;
  };
};
